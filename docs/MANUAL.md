...
- La radicación es el único trámite con `opensDirectly: true` en el registro de flujos:
  se lanza con solo mencionarla, sin la palabra de confirmación que exigen los demás.
  Puede hacerlo porque su primer paso es la pregunta de orientación, no un formulario,
  así que mencionarla no abre nada sin querer —que es el riesgo que la palabra de
  confirmación existe para evitar—. La palabra (`radicar`) se conserva en el registro
  para cuando la orientación previa está desactivada.
